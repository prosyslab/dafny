...
static class WpLoopContextCommand {
  ...
  private static readonly Option<int?> LoopLine = new("--loop-line",
    "1-based line inside the loop selected for invariant synthesis.");

  static WpLoopContextCommand() {
    ...
    OptionRegistry.RegisterOption(LoopLine, OptionScope.Cli);
  }

  public static Command Create() {
    var result = new Command("wp-loop-context",
      "Report AST-derived loop context for weakest-precondition invariant generation.");
  ...
  private static async Task<int> Execute(DafnyOptions options) {
    if (options.Get(Format) != "json") {
      await options.ErrorWriter.WriteLineAsync("wp-loop-context only supports --format json.");
      return (int)ExitValue.PREPROCESSING_ERROR;
    }
    if (options.CliRootSourceUris.Count != 1 || !options.CliRootSourceUris[0].IsFile) {
      await options.ErrorWriter.WriteLineAsync("wp-loop-context requires exactly one Dafny source file.");
      return (int)ExitValue.PREPROCESSING_ERROR;
    }
