...
namespace Microsoft.Dafny {
  ...
  interface ICanAstPrint {
    void Render(TextWriter wr, AstPrinter printer, int indent);
  }
  ...
  public partial class AstPrinter {
    ...
    public void PrintStatement(Statement stmt, int indent, bool includeSemicolon = true) {
      ...
      if (stmt.IsGhost && printMode == AstPrintModes.NoGhostOrIncludes) {
        return;
      }
      ...
      if (stmt is ICanAstPrint canPrint) {
      ...
      switch (stmt) {
        ...
        case CalcStmt calcStmt: {
            CalcStmt s = calcStmt;
            if (printMode == AstPrintModes.NoGhostOrIncludes) { return; }   // Calcs don't get a "ghost" attribute, but they are.
        ...
        case VarDeclStmt declStmt: {
            var s = declStmt;
            if (s.Locals.Exists(v => v.IsGhost) && printMode == AstPrintModes.NoGhostOrIncludes) { return; }
